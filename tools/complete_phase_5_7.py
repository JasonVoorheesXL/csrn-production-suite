...
BRANCH = "phase/5.7-graphics-obs-routes"
INTEGRATION_COMMIT = "Phase 5.7: integrate graphics and OBS route blueprints"
VERSION = "1.13.0-alpha.5g"
FEATURE = "Graphics and OBS Routes"
...
def version_present() -> bool:
    ...
    return (
        ...
        and "Version 1.13.0-alpha.5g — Graphics and OBS Routes" in app_text
        and "V1.13A5G-GRAPHICS-AND-OBS-ROUTES" in app_text
    )
...
def main() -> None:
    ...
    if not initial_paths and not integration_committed:
        print("Applying graphics and OBS route integration...")
        run((sys.executable, "tools/apply_phase_5_7.py"))
        ...
        raise CompletionError(
            "Working tree contains unexpected changes before Phase 5.7 completion: "
            + rendered
        )

    print("Running focused graphics and OBS route validation...")
    run(
        (
            sys.executable,
            "-m",
            "py_compile",
            "app.py",
            "routes/graphics_routes.py",
            "routes/obs_routes.py",
        )
    )
    run(
        (
            ...
            "pytest",
            "-q",
            "tests/test_graphics_routes_blueprint.py",
            "tests/test_obs_routes_blueprint.py",
            "tests/test_graphics_obs_route_architecture.py",
            "tests/test_phase_5_7_migration.py",
        )
    )
    ...
    if not version_present():
        ...
        print("Updating the Phase 5.7 runtime identity...")
        run(
            (
                sys.executable,
        ...
        print("Phase 5.7 runtime identity is already updated.")

    print("Running full repository validation...")
    run((sys.executable, "tools/dev_workflow.py", "validate"))

    run(("git", "rm", "tools/apply_phase_5_7.py"))
    run(("git", "rm", "tools/ci_apply.py"))
    run(("git", "rm", "tests/test_phase_5_7_migration.py"))
    run(("git", "rm", "tools/complete_phase_5_7.py"))
    run(
        (
            "git",
            "add",
    ...
    run(("git", "diff", "--cached", "--check"))
    run(("git", "commit", "-m", "Phase 5.7: complete Graphics and OBS Routes"))
    ...
    print("Phase 5.7 completion commits pushed successfully.")
    print("Expected focused total: 21 passed.")
    print("Expected final CI total after cleanup: 700 passed.")
