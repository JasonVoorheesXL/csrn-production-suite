...
BRANCH = "phase/4.18-event-service"
INTEGRATION_COMMIT = "Phase 4.18: integrate Event Service"
VERSION = "1.13.0-alpha.4r"
...
def version_present() -> bool:
    ...
    return (
        "Version 1.13.0-alpha.4r — Event Service" in version_text
        and "Version 1.13.0-alpha.4r — Event Service" in app_text
        and "V1.13A4R-EVENT-SERVICE" in app_text
    )
...
def main() -> None:
    ...
    if not initial_paths and not integration_committed:
        print("Applying EventService integration...")
        run((sys.executable, "tools/apply_phase_4_18.py"))
    elif initial_paths == {"app.py"} and integration_present():
        print("Resuming from the already-applied EventService integration...")
    elif integration_committed and initial_paths.issubset(version_paths):
        print("Resuming after the EventService integration commit...")
    elif not initial_paths and integration_committed:
        print("Resuming after the EventService integration commit...")
    else:
        rendered = ", ".join(sorted(initial_paths)) or "unknown files"
        raise CompletionError(
            "Working tree contains unexpected changes before Phase 4.18 completion: "
            + rendered
        )

    print("Running focused EventService validation...")
    run((sys.executable, "-m", "py_compile", "app.py", "event_service.py"))
    run(
        (
            sys.executable,
            "-m",
            ...
            "tests/test_phase_4_18_migration.py",
        )
    )
    ...
    if not integration_committed:
        if status_paths() != {"app.py"}:
            raise CompletionError(
                "EventService integration changed unexpected files: "
    ...
    if not version_present():
        ...
        print("Updating the Phase 4.18 runtime identity...")
        run(
            (
                sys.executable,
        ...
        print("Phase 4.18 runtime identity is already updated.")

    print("Running full repository validation...")
    run((sys.executable, "tools/dev_workflow.py", "validate"))

    run(("git", "rm", "tools/apply_phase_4_18.py"))
    run(("git", "rm", "tools/ci_apply.py"))
    run(("git", "rm", "tests/test_phase_4_18_migration.py"))
    run(("git", "rm", "tools/complete_phase_4_18.py"))
    run(
        (
            "git",
            "add",
    ...
    run(("git", "diff", "--cached", "--check"))
    run(("git", "commit", "-m", "Phase 4.18: complete Event Service"))
    ...
    print("Phase 4.18 completion commits pushed successfully.")
    print("Expected focused total: 31 passed.")
    print("Expected final CI total after cleanup: 406 passed.")
