...
BRANCH = "phase/4.23-broadcast-lifecycle-service"
INTEGRATION_COMMIT = "Phase 4.23: integrate Broadcast Lifecycle Service"
VERSION = "1.13.0-alpha.4w"
FEATURE = "Broadcast Lifecycle Service"
...
def version_present() -> bool:
    ...
    return (
        "Version 1.13.0-alpha.4w — Broadcast Lifecycle Service" in version_text
        and "Version 1.13.0-alpha.4w — Broadcast Lifecycle Service" in app_text
        and "V1.13A4W-BROADCAST-LIFECYCLE-SERVICE" in app_text
    )
...
def main() -> None:
    ...
    if not initial_paths and not integration_committed:
        print("Applying BroadcastLifecycleService integration...")
        run((sys.executable, "tools/apply_phase_4_23.py"))
    elif initial_paths == {"app.py"} and integration_present():
        print("Resuming from the already-applied BroadcastLifecycleService integration...")
    elif integration_committed and initial_paths.issubset(version_paths):
        print("Resuming after the BroadcastLifecycleService integration commit...")
    elif not initial_paths and integration_committed:
        print("Resuming after the BroadcastLifecycleService integration commit...")
    else:
        rendered = ", ".join(sorted(initial_paths)) or "unknown files"
        raise CompletionError(
            "Working tree contains unexpected changes before Phase 4.23 completion: "
            + rendered
        )

    print("Running focused BroadcastLifecycleService validation...")
    run(
        (
            sys.executable,
            "-m",
            "py_compile",
            "app.py",
            "broadcast_lifecycle_service.py",
        )
    )
    run(
        (
            ...
            "pytest",
            "-q",
            "tests/test_broadcast_lifecycle_service.py",
            "tests/test_broadcast_lifecycle_routes.py",
            "tests/test_phase_4_23_migration.py",
        )
    )
    ...
    if not integration_committed:
        if status_paths() != {"app.py"}:
            raise CompletionError(
                "BroadcastLifecycleService integration changed unexpected files: "
    ...
    if not version_present():
        ...
        print("Updating the Phase 4.23 runtime identity...")
        run(
            (
                sys.executable,
        ...
        print("Phase 4.23 runtime identity is already updated.")

    print("Running full repository validation...")
    run((sys.executable, "tools/dev_workflow.py", "validate"))

    run(("git", "rm", "tools/apply_phase_4_23.py"))
    run(("git", "rm", "tools/ci_apply.py"))
    run(("git", "rm", "tests/test_phase_4_23_migration.py"))
    run(("git", "rm", "tools/complete_phase_4_23.py"))
    run(
        (
            "git",
            "add",
    ...
    run(("git", "diff", "--cached", "--check"))
    run(("git", "commit", "-m", "Phase 4.23: complete Broadcast Lifecycle Service"))
    ...
    print("Phase 4.23 completion commits pushed successfully.")
    print("Expected focused total: 22 passed.")
    print("Expected final CI total after cleanup: 525 passed.")
