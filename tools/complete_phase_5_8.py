...
BRANCH = "phase/5.8-live-game-routes"
INTEGRATION_COMMIT = "Phase 5.8: integrate live game route blueprint"
VERSION = "1.13.0-alpha.5h"
FEATURE = "Live Game Routes"
...
def version_present() -> bool:
    ...
    return (
        ...
        and "Version 1.13.0-alpha.5h — Live Game Routes" in app_text
        and "V1.13A5H-LIVE-GAME-ROUTES" in app_text
    )
...
def main() -> None:
    ...
    if not initial_paths and not integration_committed:
        print("Applying live-game route integration...")
        run((sys.executable, "tools/apply_phase_5_8.py"))
        ...
        raise CompletionError(
            "Working tree contains unexpected changes before Phase 5.8 completion: "
            + rendered
        )

    print("Running focused live-game route validation...")
    run(
        (
            sys.executable,
            "-m",
    ...
    run(
        (
            ...
            "tests/test_live_game_route_architecture.py",
            "tests/test_phase_5_8_migration.py",
        )
    )
    ...
    if not version_present():
        ...
        print("Updating the Phase 5.8 runtime identity...")
        run(
            (
                sys.executable,
        ...
        print("Phase 5.8 runtime identity is already updated.")

    print("Running full repository validation...")
    run((sys.executable, "tools/dev_workflow.py", "validate"))

    run(("git", "rm", "tools/apply_phase_5_8.py"))
    run(("git", "rm", "tools/ci_apply.py"))
    run(("git", "rm", "tests/test_phase_5_8_migration.py"))
    run(("git", "rm", "tools/complete_phase_5_8.py"))
    run(
        (
            "git",
            "add",
    ...
    run(("git", "diff", "--cached", "--check"))
    run(("git", "commit", "-m", "Phase 5.8: complete Live Game Routes"))
    ...
    print("Phase 5.8 completion commits pushed successfully.")
    print("Expected focused total: 30 passed.")
    print("Expected final CI total after cleanup: 729 passed.")
