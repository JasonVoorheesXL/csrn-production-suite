...
BRANCH = "phase/4.17-upgrade-service"
INTEGRATION_COMMIT = "Phase 4.17: integrate Upgrade Service"
VERSION = "1.13.0-alpha.4q"
FEATURE = "Upgrade Service"
...
def integration_present() -> bool:
    ...
    return all(
        ...
        for marker in (
            ...
            "get_upgrade_service().candidate()",
            "get_upgrade_service().status()",
            "get_upgrade_service().run(",
        )
    )
...
def version_present() -> bool:
    ...
    return (
        "Version 1.13.0-alpha.4q — Upgrade Service" in version_text
        and "Version 1.13.0-alpha.4q — Upgrade Service" in app_text
        and "V1.13A4Q-UPGRADE-SERVICE" in app_text
    )
...
def main() -> None:
    ...
    # Google Drive can lock Git object folders during automatic maintenance.
    # Disable repository-local auto-GC before any commits or pushes.
    ...
    if not initial_paths and not integration_committed:
        print("Applying UpgradeService integration...")
        run((sys.executable, "tools/apply_phase_4_17.py"))
    elif initial_paths == {"app.py"} and integration_present():
        print("Resuming from the already-applied UpgradeService integration...")
    elif integration_committed and initial_paths.issubset(version_paths):
        print("Resuming after the UpgradeService integration commit...")
    elif not initial_paths and integration_committed:
        print("Resuming after the UpgradeService integration commit...")
    else:
        rendered = ", ".join(sorted(initial_paths)) or "unknown files"
        raise CompletionError(
            "Working tree contains unexpected changes before Phase 4.17 completion: "
            + rendered
        )

    print("Running focused UpgradeService validation...")
    run((sys.executable, "-m", "py_compile", "app.py", "upgrade_service.py"))
    run(
        (
            sys.executable,
            "-m",
            ...
            "tests/test_upgrade_routes.py",
            "tests/test_phase_4_17_migration.py",
        )
    )
    ...
    if not integration_committed:
        if status_paths() != {"app.py"}:
            raise CompletionError(
                "UpgradeService integration changed unexpected files: "
    ...
    if not version_present():
        ...
        print("Updating the Phase 4.17 runtime identity...")
        run(
            (
                sys.executable,
        ...
        print("Phase 4.17 runtime identity is already updated.")

    print("Running full repository validation...")
    run((sys.executable, "tools/dev_workflow.py", "validate"))

    run(("git", "rm", "tools/apply_phase_4_17.py"))
    run(("git", "rm", "tools/ci_apply.py"))
    run(("git", "rm", "tests/test_phase_4_17_migration.py"))
    run(("git", "rm", "tools/complete_phase_4_17.py"))
    run(
        (
            "git",
            "add",
    ...
    run(("git", "diff", "--cached", "--check"))
    run(("git", "commit", "-m", "Phase 4.17: complete Upgrade Service"))
    ...
    print("Phase 4.17 completion commits pushed successfully.")
    print("Expected focused total: 19 passed.")
    print("Expected final CI total after cleanup: 376 passed.")
