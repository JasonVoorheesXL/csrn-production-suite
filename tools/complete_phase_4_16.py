...
BRANCH = "phase/4.16-diagnostics-service"
INTEGRATION_COMMIT = "Phase 4.16: integrate Diagnostics Service"
VERSION = "1.13.0-alpha.4p"
FEATURE = "Diagnostics Service"
...
def integration_present() -> bool:
    ...
    return all(
        ...
        for marker in (
            ...
            "get_diagnostics_service().readiness()",
        )
    )
...
def version_present() -> bool:
    ...
    return (
        "Version 1.13.0-alpha.4p — Diagnostics Service" in version_text
        and "Version 1.13.0-alpha.4p — Diagnostics Service" in app_text
        and "V1.13A4P-DIAGNOSTICS-SERVICE" in app_text
    )
...
def main() -> None:
    ...
    if not initial_paths and not integration_committed:
        print("Applying DiagnosticsService integration...")
        run((sys.executable, "tools/apply_phase_4_16.py"))
    elif initial_paths == {"app.py"} and integration_present():
        print("Resuming from the already-applied DiagnosticsService integration...")
    elif integration_committed and initial_paths.issubset(version_paths):
        print("Resuming after the DiagnosticsService integration commit...")
    elif not initial_paths and integration_committed:
        print("Resuming after the DiagnosticsService integration commit...")
    else:
        rendered = ", ".join(sorted(initial_paths)) or "unknown files"
        raise CompletionError(
            "Working tree contains unexpected changes before Phase 4.16 completion: "
            + rendered
        )

    print("Running focused DiagnosticsService validation...")
    run((sys.executable, "-m", "py_compile", "app.py", "diagnostics_service.py"))
    run(
        (
            sys.executable,
            "-m",
            ...
            "tests/test_phase_4_16_migration.py",
        )
    )
    ...
    if not integration_committed:
        if status_paths() != {"app.py"}:
            raise CompletionError(
                "DiagnosticsService integration changed unexpected files: "
    ...
    if not version_present():
        ...
        print("Updating the Phase 4.16 runtime identity...")
        run(
            (
                sys.executable,
        ...
        print("Phase 4.16 runtime identity is already updated.")

    print("Running full repository validation...")
    run((sys.executable, "tools/dev_workflow.py", "validate"))

    run(("git", "rm", "tools/apply_phase_4_16.py"))
    run(("git", "rm", "tools/ci_apply.py"))
    run(("git", "rm", "tests/test_phase_4_16_migration.py"))
    run(("git", "rm", "tools/complete_phase_4_16.py"))
    run(
        (
            "git",
            "add",
    ...
    run(("git", "diff", "--cached", "--check"))
    run(("git", "commit", "-m", "Phase 4.16: complete Diagnostics Service"))
    ...
    print("Phase 4.16 completion commits pushed successfully.")
    print("Expected focused total: 15 passed.")
    print("Expected final CI total after cleanup: 358 passed.")
