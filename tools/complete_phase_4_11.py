...
BRANCH = "phase/4.11-graphics-service"
...
def run_focused_validation() -> None:
    print("Running focused GraphicsService validation...")
    run(
        (
            sys.executable,
            "-m",
            ...
            "app.py",
            "graphics_service.py",
            "tests/test_graphics_service.py",
            "tests/test_graphics_routes.py",
        )
    )
    run(
        (
            ...
            "tests/test_graphics_service.py",
            "tests/test_graphics_routes.py",
            "tests/test_phase_4_11_migration.py",
        )
    )
...
def main() -> None:
    ...
    if not paths and not integrated:
        print("Applying GraphicsService integration...")
        run((sys.executable, "tools/apply_phase_4_11.py"))
        integrated = integration_present()
    elif paths == {"app.py"} and integrated:
        print("Resuming from the already-applied GraphicsService integration...")
    elif not paths and integrated and not versioned:
        print("Resuming after the GraphicsService integration commit...")
    elif paths.issubset(version_paths) and integrated and versioned:
        print("Resuming after interrupted Phase 4.11 full validation...")
    elif paths:
        rendered = ", ".join(sorted(paths))
        raise CompletionError(
            "Working tree contains unexpected changes before Phase 4.11 completion: "
            + rendered
        )

    if not integrated:
        raise CompletionError("GraphicsService integration markers are incomplete.")
    ...
    if current_paths == {"app.py"}:
        ...
        run(("git", "commit", "-m", "Phase 4.11: integrate Graphics Service"))
        ...
        raise CompletionError(
            "Unexpected files after GraphicsService integration: "
            + ", ".join(sorted(current_paths))
        )

    if not version_present():
        print("Updating the Phase 4.11 runtime identity...")
        run(
            (
                sys.executable,
    ...
    temporary_files = (
        "tools/apply_phase_4_11.py",
        "tools/ci_apply.py",
        "tests/test_phase_4_11_migration.py",
        "tools/complete_phase_4_11.py",
    )
    ...
    run(("git", "diff", "--cached", "--check"))
    run(("git", "commit", "-m", "Phase 4.11: complete Graphics Service"))
    ...
    print("Phase 4.11 completion commits pushed successfully.")
    print("Expected focused total: 21 passed.")
    print("Expected final CI total after cleanup: 280 passed.")
