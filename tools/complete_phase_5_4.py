...
BRANCH = "phase/5.4-roster-personnel-venue-routes"
INTEGRATION_COMMIT = "Phase 5.4: integrate roster personnel and venue route Blueprints"
VERSION = "1.13.0-alpha.5d"
FEATURE = "Roster Personnel and Venue Routes"
...
def version_present() -> bool:
    ...
    identity = "Version 1.13.0-alpha.5d — Roster Personnel and Venue Routes"
    return (
        identity in version_text
        and identity in app_text
        and "V1.13A5D-ROSTER-PERSONNEL-AND-VENUE-ROUTES" in app_text
    )
...
def main() -> None:
    ...
    if not initial_paths and not integration_committed:
        print("Applying roster, personnel, and venue route integration...")
        run((sys.executable, "tools/apply_phase_5_4.py"))
        ...
        raise CompletionError(
            "Working tree contains unexpected changes before Phase 5.4 completion: "
            + rendered
        )

    print("Running focused roster, personnel, and venue route validation...")
    run(
        (
            sys.executable,
            "-m",
    ...
    run(
        (
            ...
            "pytest",
            "-q",
            "tests/test_personnel_routes_blueprint.py",
            "tests/test_roster_routes_blueprint.py",
            "tests/test_venue_routes_blueprint.py",
            "tests/test_roster_personnel_venue_route_architecture.py",
            "tests/test_phase_5_4_migration.py",
        )
    )
    ...
    if not version_present():
        ...
        print("Updating the Phase 5.4 runtime identity...")
        run((sys.executable, "tools/dev_workflow.py", "version", VERSION, FEATURE))
    else:
        print("Phase 5.4 runtime identity is already updated.")

    print("Running full repository validation...")
    run((sys.executable, "tools/dev_workflow.py", "validate"))

    run(("git", "rm", "tools/apply_phase_5_4.py"))
    run(("git", "rm", "tools/ci_apply.py"))
    run(("git", "rm", "tests/test_phase_5_4_migration.py"))
    run(("git", "rm", "tools/complete_phase_5_4.py"))
    run(("git", "add", "VERSION.txt", "app.py", "tests/test_core_repository_runtime.py"))
    run(("git", "diff", "--cached", "--check"))
    run(("git", "commit", "-m", "Phase 5.4: complete Roster Personnel and Venue Routes"))
    ...
    print("Phase 5.4 completion commits pushed successfully.")
    print("Expected focused total: 29 passed.")
    print("Expected final CI total after cleanup: 624 passed.")
