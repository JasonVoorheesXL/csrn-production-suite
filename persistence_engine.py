...
class PersistenceError(RuntimeError):
    """Base error for CSRN persistence failures."""


class DataCorruptionError(PersistenceError):
    """Raised when a data file cannot be decoded, validated, or recovered."""
...
class PersistencePolicy:
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 < self.max_count_drop_ratio <= 1:
            raise ValueError("max_count_drop_ratio must be greater than 0 and at most 1")


class JsonPersistenceEngine:
    """Atomic JSON persistence with validation, backups, and recovery."""
    ...
    def load(
        ...
        *,
        ...
        restore_recovered_file: bool = True,
    ) -> Any:
        path = Path(path)
        with self._lock:
            if not path.exists():
            ...
            try:
                ...
                self._validate(path, data, validator)
                return data
            except (OSError, json.JSONDecodeError, DataCorruptionError) as exc:
                quarantined = self._quarantine(path)
                recovered = self._recover_latest(path, validator)
                if recovered is None:
                    raise DataCorruptionError(
                        f"Could not recover {path}. Damaged copy preserved at {quarantined}."
                    ) from exc
                if restore_recovered_file:
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                staged = self._decode(temp_path)
                self._validate(path, staged, validator)
                if path.exists():
                    self._snapshot(path)
    ...
    @staticmethod
    def _validate(path: Path, data: Any, validator: Callable[[Any], bool] | None) -> None:
        if validator is not None and not validator(data):
            raise DataCorruptionError(f"Validation failed for {path}.")
    ...
    def _enforce_destructive_guard(
        ...
        if existing is None:
            return
        old_count = self.record_count(existing)
        new_count = self.record_count(incoming)

        if policy.block_empty_replacement and old_count > 0 and new_count == 0:
            raise DestructiveWriteBlocked(
                f"Blocked empty replacement of populated data file {path}. "
                "Use force=True only for an intentional reset."
            )
        ...
        if policy.block_large_count_drop and old_count > 0:
            ...
            if drop_ratio >= policy.max_count_drop_ratio:
                raise DestructiveWriteBlocked(
                    f"Blocked suspicious record-count drop for {path}: "
    ...
    def _quarantine(self, path: Path) -> Path:
        ...
        destination = self.quarantine_root / (
            f"{path.stem}-corrupt-{stamp}-{time.time_ns()}{path.suffix}"
        )
        shutil.copy2(path, destination)
        return destination
    ...
    def _recover_latest(
        ...
        backups = sorted(
            self._backup_dir(path).glob("*.json"),
            key=lambda candidate: candidate.stat().st_mtime_ns,
            reverse=True,
        )
        for candidate in backups:
            try:
                data = self._decode(candidate)
                self._validate(candidate, data, validator)
                return data
            except (OSError, json.JSONDecodeError, DataCorruptionError):
                continue
        return None
